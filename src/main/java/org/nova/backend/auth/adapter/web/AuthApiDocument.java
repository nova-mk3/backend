...
public @interface AuthApiDocument {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "새로운 회원 생성 성공",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "400", description = "잘못된 요청 데이터",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "500", content = @Content(mediaType = "application/json",
                    examples = {@ExampleObject(name = "학번 중복",
                            value = "{\"code\": 500, \"message\": \"Member already exists 20202020\"}"),
                            @ExampleObject(name = "이메일 중복",
                                    value = "{\"code\": 500, \"message\": \"duplicated entry 이메일\"}"),
